 ...
 *     runDueInterviewReminders / runDueScheduledReports / runDueTimeInStageWorkflows
 *
 * Deliberately not a generic polymorphic job-queue table — the three
 ...
 * stage-age threshold) that forcing them into one shared "ScheduledJob" row
 * shape would only be a paraphrase of what each already does well on its
 * own, not a simplification. This function's only job is invocation,
 * isolation, and reporting — each consumer owns its own idempotency,
 * batching, and retry logic (see their own files).
 *
